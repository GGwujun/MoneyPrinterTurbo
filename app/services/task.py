...
# 跨平台发布（cross-post）编排与视频配乐供应商表已分别抽到独立模块，
# 让本文件聚焦视频生成流水线。这里 re-export 保留向后兼容：外部调用方
# （asgi / controllers / webui）和现有测试仍可通过 task.<symbol> 访问这些名字。
from app.services.cross_post import (  # noqa: F401
...
from app.services.music_provider import (  # noqa: F401
    _VIDEO_MUSIC_PROVIDERS,
    _get_video_music_prompt,
)
